...
export const problems: Problem[] = [
  ...
  },
  {
    ...
    context: "SA's 12 official languages create confusion in classrooms, with English-Moderate teaching leaving non-native speakers behind, especially in diverse urban schools like those in Johannesburg. In 2025, post-COVID learning gaps widen this issue, and rural areas lack isiZulu or Setswana materials. The DBE's multilingual policy pushes for mother-tongue education, but tools are scarce.",
    keyChallenges: [
...
export const PROBLEM_IMAGES: Record<number, string> = {
  1: "/src/assets/loadshedding.png",
  2: "/src/assets/no water.png",
  3: "/src/assets/walking stuudent.png",
  4: "/src/assets/language barrier.png",
  5: "/src/assets/practical skills.png",
  6: "/src/assets/bully.png",
  7: "/src/assets/mental health.png",
  8: "/src/assets/disabled kids.png",
};
...
export const generateAIResponse = (type: 'interview' | 'refine' | 'wildcard' | 'score', context?: any) => {
  if (type === 'interview') {
    const b1Responses = [
      "That is a good question. I think the main problem is often about how people feel. It can be hard to change things.",
      "I understand your point. Many people face similar issues. We need to find simple ways to help them.",
      "Yes, that is true. This situation makes things difficult for everyone. We should look for practical solutions.",
      "Thank you for asking. I believe the key is to listen to others. Then, we can work together to make things better.",
      "It is important to think about this. People want things to be easy to use. We must keep this in mind."
    ];
    return b1Responses[Math.floor(Math.random() * b1Responses.length)];
  }
  if (type === 'refine') {
    return {
...
export const mockLogin = async (email: string, password: string, avatarUrl?: string) => {
  await new Promise(resolve => setTimeout(resolve, 500));
  const token = btoa(JSON.stringify({ email, exp: Date.now() + 86400000 }));
  return {
    id: 'mock_user_id_' + Math.random().toString(36).substr(2, 9), // Added mock ID
    username: email.split('@')[0],
    email,
    grade: "10th Grade",
    avatar: avatarUrl || "/models/Armature.glb", // Use provided avatarUrl or default
    authToken: token
  };
};

export const mockSignup = async (username: string, email: string, password: string, grade: string, avatarUrl: string) => {
  await new Promise(resolve => setTimeout(resolve, 500));
